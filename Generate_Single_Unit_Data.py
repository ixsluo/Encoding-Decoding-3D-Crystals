            ...
            for j in range(30):
                for k in range(30):
                    ...
                    for x in range(len(XYZs_shifted)):
                        ...
                        mat[i, j, k] += 1.0 / ((2.0 * np.pi)**1.5) * Zs[x] * (
...
if __name__ == '__main__':
    ...
    np.random.seed(1)
    file_list = np.random.permutation(np.arange(0, upper))
    start = delta * rank
    stop = start + delta
    if rank == (size - 1):
    ...
    for i in range(start, stop):
        ...
        electron_density, species_mat, Zs, XYZs_shifted = generate(
            crystal, 0, 0, 0)
        if len(np.shape(electron_density)) != 0:
            # Ignore things that were [0,0,0,0]
            to_save.append([
                file_list[i], electron_density, species_mat,
                target[file_list[i]], Zs, XYZs_shifted
            ])

    if saveQ == True:
        with open(directory + 'SAVE_Unit/Unit_' + str(rank) + '.pickle',
                  'wb') as f:
            print('SAVING ', rank)
