...
def generate(crystal, a, b, c):
    ...
    '''
    Atomic_Numbers = [
        Element(crystal[i].species_string).Z for i in range(len(crystal))
    ]
    ...
    for i in range(30):
        for j in range(30):
            for k in range(30):
                s = PeriodicSite(1, [bins[i] + a, bins[j] + b, bins[k] + c],
                                 lattice,
                                 coords_are_cartesian=True)
                for x in range(len(crystal)):
                    (dist, im) = crystal[x].distance_and_image(s)
                    mat[i, j, k] += 1.0 / (
                        (2.0 * np.pi)**1.5) * Atomic_Numbers[x] * (
                            1.0 / sigma**3) * np.exp(-dist**2 / (2 * sigma**2))
                    if dist < 0.65:
                        if mat2[i, j, k] > 0.0:
...
if __name__ == '__main__':
    ...
    files = glob.glob(directory + 'crys_hoffmann/' + '*')
    np.random.seed(1)
    ...
    to_save = []
    with tqdm(total=(stop-start), desc=str(rank)) as pbar:
        for i in range(start, stop):
            if rank == 0:
                print("I am on " + str(i) + " and I am going to " + str(stop))
            crystal = get_structure(directory + 'crys_hoffmann/' +
                                    str(file_list[i]) + '.cif')
            abc = np.array(crystal.lattice.abc)
            if min(abc) < 10:
                electron_density, electron_density2 = generate(crystal, 0, 0, 0)
                to_save.append([
                    file_list[i], electron_density, electron_density2,
                    target[file_list[i]]
                ])
                electron_density, electron_density2 = generate(
                    crystal, 10 * np.random.rand(), 10 * np.random.rand(),
                    10 * np.random.rand())
                to_save.append([
                    file_list[i], electron_density, electron_density2,
                    target[file_list[i]]
        ...
        if saveQ == True:
            with open(directory + 'SAVE/S_3x_' + str(rank) + '.pickle', 'wb') as f:
                print('SAVING')
